...
func (f *FederationEnvoy) handlePendingProofPushes() error {
	...
	pushTimeout := f.cfg.SyncInterval - time.Second
	if pushTimeout <= 0 {
		pushTimeout = f.cfg.SyncInterval
	}
	if pushTimeout <= 0 {
		pushTimeout = DefaultTimeout
	}

	for idx := range logEntries {
		entry := logEntries[idx]

		ctxPush, cancelPush := f.CtxBlockingCustomTimeout(pushTimeout)
...
func (f *FederationEnvoy) queueProofPushes(ctx context.Context,
	...
	if err != nil {
		return fmt.Errorf("unable to fetch federation servers: %w", err)
	}
	if len(fedServers) == 0 {
		return nil
	}
	...
	for idx := range items {
		...
		for serverIdx := range pendingServers {
			...
			if err != nil {
				return fmt.Errorf("unable to queue federation push: %w",
					err)
			}
